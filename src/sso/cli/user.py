...
def create(
    ...
    with get_db() as session:
        ...
        user = User(
            email=email,
            hashed_password=get_password_hasher().hash_password(password),
            tenant=tenant_obj,
        )
        session.add(user)
        session.commit()
...
def list_tenants(tenant_name: str = TenantOption) -> None:
    ...
    with get_db() as session:
        for tenant in (
            session.query(User).join(Tenant).filter(Tenant.name == tenant_name).all()
        ):
            users.append(tenant)
...
def login(
    *,
    ...
    with get_db() as session:
        ...
        access_token, refresh_token = token_context.create_tokens(user=user)
        ...
        if verify:
            ...
            print(jws.verify_signature(tenant.get_public_key(), alg=JWT_ALGORITHM))
    return 0
